"""Purpose: Reconstruct full speaker tracks from Cholimex regions.

Inputs: Mixture waveform, classified regions, separator and embeddings.
Outputs: Two timeline-aligned reconstructed speaker waveforms.
"""
...
def reconstruct_tracks(
    ...
    debug_callback: Callable[[dict, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, int], None] | None = None,
    ...
    for region in regions:
        ...
        if debug_callback is not None:
            debug_callback(record, original[:, start:end], cand_0, cand_1, assigned[0], assigned[1], sample_rate)
